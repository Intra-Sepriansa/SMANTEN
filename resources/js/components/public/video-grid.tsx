...
const categories = ['Semua', 'Budaya', 'Kepemimpinan', 'Media', 'Lingkungan', 'Sekolah'];
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isActive ? "text-white" : "text-[var(--school-muted)] hover:text-[var(--school-ink)]"
                            )}
                        >
                            {isActive && (
                                <motion.div
                ...
                >
                    {items.length > 0 ? (
                        items.map((item, index) => {
                            ...
                            const localThumb = item.thumbnailUrl || fallbacks[index % fallbacks.length];
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <div className="relative overflow-hidden p-6 min-h-[14rem] flex flex-col justify-between shrink-0">
                                            {/* Background Image that scales independently on hover */}
                                            <div 
                                                className="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110"
                                                style={{ backgroundImage: `url(${localThumb})` }}
                                            />
                                            ...
                                            <div className="relative z-10 flex items-center justify-between text-white">
                                                <span className="rounded-full border border-white/30 bg-white/10 px-4 py-1.5 text-[0.68rem] font-bold uppercase tracking-[0.26em] backdrop-blur-md">
                                                    {item.category}
                                                </span>
                                            ...
                                            <div className="relative z-10 mt-10 flex items-end justify-between">
                                                <div>
                                                    <div className="text-[0.65rem] font-bold uppercase tracking-widest text-[#0E9EE4] drop-shadow-sm">
                                                        {item.state}
                                                    </div>
                                                    <div className="mt-2 text-2xl font-bold leading-tight text-white drop-shadow-md">
                                                        {item.title}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="relative z-20 space-y-4 bg-white/95 p-6 backdrop-blur-xl shrink-0 grow">
                                            <p className="text-sm leading-7 text-[var(--school-muted)]">
                                                {item.description}
                                            </p>
                                            <div className="flex items-center justify-between pt-2">
                                                <div className="text-[0.65rem] font-bold uppercase tracking-[0.28em] text-[var(--school-green-700)]">
                        ...
                        <div className="rounded-[1.8rem] border border-dashed border-[var(--school-green-200)] bg-white/72 p-8 text-sm leading-7 text-[var(--school-muted)] md:col-span-2 xl:col-span-4">
                            Feed video publik belum tersedia. Backend sudah membuka kontrak
                            YouTube dan fallback kurasi, sehingga kartu ini tinggal menerima
                            channel resmi saat konfigurasi API dilengkapi.
                        </div>
                    )}
                </motion.div>
            )}
